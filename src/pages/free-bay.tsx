...
export default function FreeBayLandingPage() {
  const router = useRouter();
  useEffect(() => {
    // All CTAs route to free bay funnel only
    const freeBayCtaTexts = ['BOOK YOUR FREE', 'CLAIM YOUR FREE BAY', 'FREE 30 MINUTE', 'BOOK NOW'];

    document.querySelectorAll('button').forEach(function(btn) {
      const t = (btn.textContent || '').trim().toUpperCase();

      // All CTAs route to free bay
      ...
      if (freeBayCtaTexts.some(function(k: string) { return t.includes(k); })) {
